...
def uniformCostSearch(problem):
    ...
    my_dict["state"]=state
    my_dict["cost"]=0
    priority_Queue.push(my_dict,my_dict["cost"])

    while not priority_Queue.isEmpty():
        my_dict=priority_Queue.pop()
        state=my_dict["state"]
        cost=my_dict["cost"]
        ...
        for suc in problem.getSuccessors(state):
            if not visited.__contains__(suc[0]):
                my_dict2={}
                my_dict2["pred"]=my_dict
                my_dict2["state"]=suc[0]
                my_dict2["act"]=suc[1]
                my_dict2["cost"]=suc[2]+cost
                priority_Queue.push(my_dict2,my_dict2["cost"])
    actions=[]
    while my_dict["act"] !=None:
        actions.insert(0,my_dict["act"])
        my_dict=my_dict["pred"]
    return actions
    util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not Pr_q.isEmpty():
        ...
        if visited.__contains__(state):  # But now for cost we are having the cost + heuristic combined
            continue
        visited[state] = True
        if problem.isGoalState(state) == True:
            break
